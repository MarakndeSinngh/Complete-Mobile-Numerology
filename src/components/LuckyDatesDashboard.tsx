...
import { formatDateIndian, parseIndianDate, normalizeIndianDate } from '../utils/dateUtils';
import { calculateMulank, calculateBhagyank } from '../core/numerologyEngine';

interface LuckyDatesDashboardProps {
  initialDob?: string;
  initialName?: string;
  onReportGenerated?: (report: LuckyDatesFinderReport) => void;
}

export const LuckyDatesDashboard: React.FC<LuckyDatesDashboardProps> = ({
  ...
  // Active View Tab
  const [activeTab, setActiveTab] = useState<'CALENDAR' | 'COMPARISON' | 'DOSSIER' | 'TRANSIT'>('CALENDAR');
  ...
  return (
    <div id="lucky-dates-finder-pro-portal" className="space-y-8 text-left font-sans text-[#1F2937]">
        ...
        <div className="relative z-10 space-y-3">
          <div className="flex flex-wrap items-center gap-2">
            <span className="bg-amber-400/20 text-amber-300 border border-amber-400/30 px-3 py-1 rounded-full text-[10px] font-mono font-bold uppercase tracking-wider flex items-center gap-1.5">
              <Calendar className="w-3.5 h-3.5 text-amber-400" /> Phase 12 Pro Module
            </span>
            <span className="bg-white/10 text-white px-3 py-1 rounded-full text-[10px] font-mono">
              Transit Matching & Auspicious Dates Finder
            </span>
          </div>

          <h2 className="font-playfair text-2xl sm:text-3xl font-bold">
            LeoFamily Lucky Dates Finder Pro (शुभ मुहूर्त व तिथि चयन वेधशाला)
          </h2>
          <p className="text-blue-100 text-xs sm:text-sm max-w-3xl leading-relaxed">
            मूलांक, भाग्यांक, व्यक्तिगत वर्ष (Personal Year), व्यक्तिगत माह एवं वार-अधिष्ठाता ग्रहों के सामंजस्य से किसी भी महत्वपूर्ण कार्य, व्यवसाय, गृह प्रवेश या विवाह के लिए शास्त्रसम्मत शुभ तिथियों का वैज्ञानिक विश्लेषण।
          </p>
        </div>
      </div>
        ...
        <div className="border-b border-[#E5E7EB] pb-4 flex flex-col sm:flex-row justify-between items-start sm:items-center gap-2">
          <div>
            <h3 className="font-playfair font-bold text-lg text-[#1E3A8A]">
              तिथि चयन इनपुट एवं कार्य विवरण (Date Finder Criteria)
            </h3>
            <span className="text-[11px] text-slate-500">
              अपनी जन्मतिथि और कार्य का उद्देश्य चुनें (All dates strictly DD/MM/YYYY)
            </span>
          </div>
          <button
          ...
          >
            ...
            {isCalculating ? 'विश्लेषण जारी...' : 'शुभ तिथियां खोजें (Scan Dates)'}
          </button>
        </div>
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4 text-xs">
          ...
          <div className="space-y-1.5">
            ...
            <span className="text-[10px] text-slate-400 block font-mono">
              Driver: #{calculateMulank(dob)} • Conductor: #{calculateBhagyank(dob)}
            </span>
          </div>
          ...
          <div className="space-y-1.5">
            <label className="font-mono text-[10px] uppercase font-bold text-slate-600 block">
              व्यक्ति / संस्था का नाम (Name)
            </label>
            <input
              type="text"
              value={name}
              onChange={(e) => setName(e.target.value)}
              placeholder="e.g. Ramesh Kumar / Leo Enterprises"
          ...
          <div className="space-y-1.5 sm:col-span-2">
            <label className="font-mono text-[10px] uppercase font-bold text-slate-600 block">
              कार्य का उद्देश्य (Selected Purpose) *
            </label>
            <select
              value={purpose}
            ...
            <span className="text-[10px] text-amber-800 font-sans block">
              {PURPOSE_DEFINITIONS[purpose]?.coreThemeHi}
            </span>
          </div>
        </div>
        ...
        <div className="p-4 bg-slate-50 border border-slate-200 rounded-3xl space-y-3">
          <div className="flex items-center justify-between">
            <span className="font-bold text-xs text-[#1E3A8A] flex items-center gap-1.5">
              <Calendar className="w-3.5 h-3.5 text-[#D97706]" /> समय सीमा चयन (Date Range Selection)
            </span>
      ...
      {report && (
        <div className="grid grid-cols-2 sm:grid-cols-4 lg:grid-cols-6 gap-3 text-center text-xs">
          ...
          <div className="p-4 bg-white rounded-3xl border border-slate-200 shadow-xs space-y-1">
            ...
            <span className="text-[10px] text-slate-600 block truncate">{report.currentPersonalYearThemeHi.slice(0, 12)}...</span>
          </div>
          ...
          <div className="p-4 bg-white rounded-3xl border border-slate-200 shadow-xs space-y-1">
            <span className="text-[9px] font-mono uppercase text-slate-500 font-bold block">कुल तिथियां (Scanned)</span>
          ...
          <div className="p-4 bg-amber-50 rounded-3xl border border-amber-200 shadow-xs space-y-1">
            <span className="text-[9px] font-mono uppercase text-amber-800 font-bold block">शीर्ष अनुकूल (Top Picks)</span>
      ...
      {/* TOP SUPPORTIVE DATES SPOTLIGHT */}
          ...
          <div className="flex justify-between items-center">
            <div className="space-y-0.5">
              <span className="text-[10px] font-mono uppercase text-[#D97706] font-bold tracking-wider">
                Auspicious Recommendations (प्राथमिक अनुशंसित तिथियां)
              </span>
            ...
            <span className="text-xs font-mono bg-white border border-amber-300 text-amber-900 px-3 py-1 rounded-full font-bold">
              {report.topSupportiveDates.length} Auspicious Dates Found
            </span>
          </div>
      ...
      <div className="flex flex-wrap items-center gap-2 border-b border-slate-200 pb-2">
        {[
          { id: 'CALENDAR', label: 'समस्त तिथियां व ग्रिड (Full Candidate List)', icon: Calendar },
          { id: 'COMPARISON', label: `तिथि तुलना मैट्रिक्स (${comparisonDates.length})`, icon: Sliders },
          { id: 'TRANSIT', label: 'ट्रांजिट मैचिंग व व्यक्तिगत चक्र', icon: Compass },
          { id: 'DOSSIER', label: '13-सूत्रीय वैदिक मुहूर्त रिपोर्ट (13-Point Dossier)', icon: FileText }
        ].map(tab => (
          <button
            key={tab.id}
      ...
      {activeTab === 'CALENDAR' && (
        <div className="space-y-6">
          ...
          <div className="p-4 bg-slate-50 rounded-2xl border border-slate-200 flex flex-wrap items-center justify-between gap-3 text-xs">
            <div className="flex flex-wrap items-center gap-2">
              <span className="font-mono text-[10px] text-slate-500 uppercase font-bold flex items-center gap-1">
                <Filter className="w-3 h-3 text-[#D97706]" /> अनुकूलता फ़िल्टर:
              </span>
              {[
                { id: 'ALL', label: 'सभी' },
            ...
            <div className="text-[11px] text-slate-500">
              प्रदर्शित: <strong>{filteredCandidates.length}</strong> / {report?.candidates.length || 0} तिथियां
            </div>
          </div>
          ...
          <div className="glass-panel rounded-[32px] bg-white border border-[#E5E7EB] shadow-sm overflow-hidden">
            <div className="overflow-x-auto">
              <table className="w-full text-left text-xs border-collapse">
                <thead>
                  <tr className="bg-slate-50/80 border-b border-slate-200 text-slate-600 font-mono text-[10px] uppercase tracking-wider">
                    <th className="py-3 px-4">तारीख (Date)</th>
                    <th className="py-3 px-4">वार व अधिष्ठाता (Day & Lord)</th>
                    <th className="py-3 px-4">तारीख अंक / योग</th>
                    <th className="py-3 px-4">Personal Day</th>
                    <th className="py-3 px-4">अनुकूलता स्तर</th>
                    <th className="py-3 px-4">विश्लेषण (Why this date?)</th>
                    <th className="py-3 px-4 text-center">तुलना</th>
                  </tr>
                </thead>
                    ...
                    return (
                      ...
                      >
                        ...
                        <td className="py-3.5 px-4 font-mono">
                          <span className="font-bold text-slate-800">#{c.dateNumber}</span>
                          <span className="text-[10px] text-slate-500 block">Root #{c.rootNumber}</span>
                        </td>
                        <td className="py-3.5 px-4">
                          <span className="font-mono font-bold text-indigo-700">#{c.personalDay}</span>
          ...
          {/* Detailed Drawer for Selected Date */}
          {selectedDetailDate && (
            <div className="p-6 bg-[#FAF5EE] rounded-3xl border border-[#FDE68A] space-y-4 text-xs">
                ...
                <div className="flex items-center gap-3">
                  ...
                  <div>
                    <h4 className="font-playfair font-bold text-base text-[#1E3A8A]">
                      तारीख विश्लेषण: {selectedDetailDate.dateFormatted} ({selectedDetailDate.weekdayHi})
                    </h4>
                    <span className="text-[11px] text-[#78350F]">
                      अधिष्ठाता: {selectedDetailDate.weekdayLord} • {selectedDetailDate.compatibilityStatusHi}
                    </span>
                  </div>
                </div>
                <button
              ...
              <div className="grid grid-cols-1 md:grid-cols-3 gap-3">
                <div className="p-3 bg-white/80 rounded-2xl border border-amber-200 space-y-1">
                  <strong className="text-slate-800 block font-bold">सकारात्मक कारक (Supporting Factors):</strong>
                ...
                <div className="p-3 bg-white/80 rounded-2xl border border-amber-200 space-y-1">
                  <strong className="text-slate-800 block font-bold">सावधानी एवं ध्यान बिंदु (Cautions):</strong>
                  <ul className="list-disc list-inside text-[11px] text-slate-600 space-y-0.5">
                ...
                <div className="p-3 bg-white/80 rounded-2xl border border-amber-200 space-y-1">
                  <strong className="text-slate-800 block font-bold">पारंपरिक मुहूर्त व समय सलाह:</strong>
          ...
          {comparisonDates.length === 0 ? (
            <div className="p-12 text-center bg-slate-50 border border-dashed border-slate-300 rounded-3xl space-y-3">
              ...
              <p className="text-xs text-slate-500 max-w-md mx-auto">
                समस्त तिथियां सूची में जाकर किसी भी तारीख के आगे '+ तुलना' बटन दबाएं।
              </p>
            </div>
          ) : (
              ...
              {comparisonDates.map((item, idx) => (
                <div key={idx} className="glass-panel p-5 bg-white rounded-3xl border border-[#E5E7EB] shadow-sm space-y-4">
                  ...
                  <div className="space-y-1.5 text-[11px] pt-1">
                    <strong className="text-slate-800 block font-bold">प्रमुख कारक:</strong>
      ...
      {/* TAB 3: TRANSIT MATCHING GRID */}
      {activeTab === 'TRANSIT' && report && (
        <div className="glass-panel p-6 sm:p-8 rounded-[36px] bg-white border border-[#E5E7EB] shadow-md space-y-6">
          <div className="border-b pb-4">
            <span className="text-[10px] font-mono uppercase text-[#D97706] font-bold tracking-wider">
              Transit & Time-Cycle Synthesis
            </span>
            <h3 className="font-playfair font-bold text-lg text-slate-900">
          ...
          {/* 7-Day Planetary Lord Reference */}
      ...
      {/* TAB 4: 13-POINT COMPLETE DOSSIER */}
...
export default LuckyDatesDashboard;
